...
import { EpisodeRow, EpisodeGridCard, ViewToggle, useViewMode, deriveStatus, useTrackProject } from "./AllEpisodes";
...
export default function Drafts() {
  ...
  return (
    <div>
      <PageHeader
        title="Drafts"
        description="Episodes still in production — pending, running, awaiting review, or failed."
      ...
      {projects.length > 0 && viewMode === "grid" && (
        <div
          role="list"
          aria-label="Grid of draft episodes"
          style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 14 }}
        >
          {projects.map((p) => (
            <EpisodeGridCard key={p.id} p={p} onTrack={trackProject} />
          ))}
        </div>
      )}
    </div>
