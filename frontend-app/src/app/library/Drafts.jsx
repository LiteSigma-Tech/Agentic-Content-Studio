...
export default function Published() {
  ...
  return (
    <div>
      ...
      {projects.length > 0 && (
        <Panel style={{ padding: 16 }} role="list" aria-label="List of published episodes">
          {projects.map((p) => (
            <div key={p.id} role="listitem" style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ flex: 1 }}>
                <EpisodeRow p={p} />
              </div>
              <Btn 
                ...
                aria-label={`Watch episode ${p.title || p.id}`}
              >
                Watch
              </Btn>
            </div>
